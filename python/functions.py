...
def message(phoneNumber, adres):
    ...
    message = client.messages.create(
        to="+31"+ phoneNumber + "" , 
        from_="+19182654384",
        body="Je bent bijna aan de beurt bij de winkel op "+adres)
...
def shopAdres(shopID):
    mycursor = connection.cursor()
    mycursor.execute("SELECT Adress FROM registerd_shops WHERE id = \'"+ str(shopID) +"\'")
    myresult = mycursor.fetchall()
    for x in myresult:
        adres = x[0]
    connection.commit()
...
def insertWachtrij(shopID, ownerID):
    mycursor = connection.cursor()
    sql = "INSERT INTO wachtrij (shopID, user_id, physical) VALUES (%s, %s, %s)"
    val = (shopID, ownerID, True)
    mycursor.execute(sql, val)
    connection.commit()

# Uncommenten wanneer je deze python file op de arduino gooit waar je kan inchecken
#
...
def setWachtrijPhysical(shopID, ownerID):
    mycursor = connection.cursor()
    sql = "UPDATE wachtrij set physical = True WHERE user_id = %s AND shopID = %s"
    val = (ownerID, shopID)
    mycursor.execute(sql,val)
    connection.commit()